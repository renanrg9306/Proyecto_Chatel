...
class PDF extends FPDF
{
		function Header()
		{
			...
			$this->Cell(80);
			$this->Cell(50,20,'Reporte de Asignaciones por Profesor',0,0,'C');
			$this->Ln(15);
		    ...
		    $profesor = $_POST['profesor'];
            $asignaciones = mysqli_query( $conex,"SELECT concat(NombresProfesor, ' ' ,ApellidosProfesor) as Profesor FROM profesor where idProfesor = '$profesor'");
            while($row = mysqli_fetch_row($asignaciones)){
            $NombreProfesor = $row[0];

            }
		     $this->Cell(90,20, $NombreProfesor, 0,0,'R');
			$this->Ln(15);
       ...
       where profesor.idProfesor = '$profesor' ");
